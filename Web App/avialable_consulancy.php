...
$sql="select *from consulancy where status=1";
$data=mysqli_query($conn,$sql);

if (isset($_POST['ignore'])) {
	$id=$_POST['id'];
	$sql="delete from consulancy where case_id='$id'";
	$rund=mysqli_query($conn,$sql);
	if($rund){
			echo '<script type="text/javascript">';
    	   	  	   echo 'alert("You Successfully Deleted");';
    	   	  	   echo 'window.location.href=avialable_consulancy.php"';
    	   	  	   echo '</script>';
	}
...
if (isset($_POST['start_track'])) {
	$id=$_POST['id'];	
	$sqlw="update consulancy set status=2 where case_id='$id'";
...
if(isset($_POST['upload'])){
	...
	$size=$_FILES['file']['size'];
		...
		if(move_uploaded_file($file, $destination)){
			$sq="insert into files(file_name,size,download,status) values('$filename.','$size',0,3)";
...
<head>
	<meta charset="utf-8">
	<title>This Is Avialable Consulancy Page</title>
...
<div class="container">
    ...
    <label for="delet" class="close_btn" title="close">&times;</label>
     <h1>Delete Case</h1>
    <form class="delete" action="avialable_consulancy.php" method="post">
  ...
  <div class="container3">
    ...
    <label for="upload" class="close_btn" title="close">&times;</label>
     <h1>Upload File</h1>
    	<form class="delete" action="avialable_consulancy.php" method="post" enctype="multipart/form-data">
    ...
    <div class="content">
		...
		if($data->num_rows >0){
			?>
			<h1><strong>Avialable Consultancy Cases</strong></h1>
			<?php
...
<input type="checkbox" id="start">
  ...
  <label for="upload" class="show_btn">Upload File</label>
    <br>
    <a href="consulancy_register.php">Add New</a></br>
    	<a href="consulancy_register.php">Edit</a>
  </td>
                    </tr>
                 ...
                 echo'<h1 class="no"><strong>No Avialable Consultancy Cases</strong></h1>';
                  ?>
...
<input type="checkbox" id="start">
  ...
  <label for="upload" class="show_btn">Upload File</label>
    <br>
    <a href="consulancy_register.php">Add New</br></a>
    	<a href="consulancy_register.php">Edit</a>
    </td></tr>
    </table>
    	<?php
    		...
    		$sqll="select *from files where status=3";
