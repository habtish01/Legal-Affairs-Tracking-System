...
$sql="select *from crime_register where status=2";
$data=mysqli_query($conn,$sql);
if(isset($_POST['update'])){
	$id=$_POST['id'];
	$status=$_POST['status'];
	$sql="update crime_register set case_description='$status' where case_id='$id'";
		$run=mysqli_query($conn,$sql);

		if($run){
			echo '<script type="text/javascript">';
    	   	  	   echo 'alert("You Successfully Updated");';
    	   	  	   echo 'window.location.href="tracked.php"';
    	   	  	   echo '</script>';
...
if (isset($_POST['finish_track'])) {
	$conn=mysqli_connect("localhost","root","","db");
	$id=$_POST['id'];
	$sql="update  crime_register set status=3 where case_id='$id'";
	$run=mysqli_query($conn,$sql);
	if($run){
		echo '<script type="text/javascript">';
...
if(isset($_POST['upload'])){
	...
	$size=$_FILES['file']['size'];
		...
		if(move_uploaded_file($file, $destination)){
			$sq="insert into files(file_name,size,download,status) values('$filename.','$size',0,10)";
...
<head>
	<meta charset="utf-8">
	<title>This Is Tracked Crime Page</title>
...
<div class="container">
    ...
    <label for="delet" class="close_btn" title="close">&times;</label>
     <h1>Delete Case</h1>
    <form class="delete" action="tracked.php" method="post">
  ...
  <div class="container3">
    ...
    <label for="upload" class="close_btn" title="close">&times;</label>
     <h1>Upload File</h1>
    	<form class="delete" action="tracked.php" method="post" enctype="multipart/form-data">
    ...
    <div class="content">
		...
		if($data->num_rows >0){
			?>
			<h1><strong>Avialable Tracked Crime Cases</strong></h1>
			<?php
...
<input type="checkbox" id="start">
  ...
  <label for="remined" class="show_btn">Set Reminder</label>
    <br>
    	<a href="register_crime.php">Edit</a>
                 ...
                 echo'<h1 class="no"><strong>No Avialable Tracked Crime Cases</strong></h1>';
                   ?>
  ...
  <label for="upload" class="show_btn">Upload File</label>
    	...
    	<?php
    		...
    		$sqll="select *from files where status=10";
