...
if (isset($_POST['register'])) {
             ...
             $id=$_POST['id'];
		      ...
		      $body=$_POST['body'];
		      $sql="insert into descipline values ('$id','$name','$date','$pname','$case_status','$body',1)";
		       $run=mysqli_query($conn,$sql);
		       if($run){
		       	echo '<script type="text/javascript">';
    	   	  	   echo 'alert("Registeration successfullly");';
    	   	  	   echo 'window.location.href="descipline_register.php"';
    	   	  	   echo '</script>';
    	   	  	}
    	   	  	else{
    	   	  		echo '<script type="text/javascript">';
...
if (isset($_POST['edit'])) {
             ...
             $id=$_POST['id'];
		      ...
		      $case_status=$_POST['case_status'];
		      $body=$_POST['body'];
		       $sql="update descipline set case_name='$name',case_date='$date',name='$pname',case_status='$case_status' ,body='$body' where case_id='$id'";
		       $run=mysqli_query($conn,$sql);
		       if($run){
		       	echo '<script type="text/javascript">';
...
<body>
  ...
  <label for="delet" class="show_btn">Send Mail</label></h2>
 		...
 		<form action="descipline_register.php" method="post">
 			<h1><u>Register A New Descipline Case Here</u></h1>
 			...
 			<input type="text" name="name" placeholder="Enter Name Of The Person" required="">
