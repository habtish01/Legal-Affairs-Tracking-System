...
if (isset($_POST['register'])) {
		...
		$body=$_POST['body'];
		$sql="insert into other_civil values ('$id','$name','$date','$acuser','$clinetent','$place','$case_status','$body',1)";
		$run=mysqli_query($conn,$sql);
		if($run){
			echo '<script type="text/javascript">';
    	   	  	   echo 'alert("Registeration Successfullly");';
    	   	  	   echo 'window.location.href="register_other.php"';
    	   	  	   echo '</script>';
		}
		else{
			echo '<script type="text/javascript">';
...
if (isset($_POST['edit'])) {
		...
		$case_status=$_POST['case_status'];
		$body=$_POST['body'];
		$sql="update other_civil set case_name='$name', case_date='$date', acuser='$acuser', clientent='$clinetent', place='$place', case_status='$case_status', case_description='$body' where case_id='$id'";
		$run=mysqli_query($conn,$sql);
			if($run){
			echo '<script type="text/javascript">';
...
<head>
	<meta charset="utf-8">
	<title>Register Civil Page</title>
...
<body>
  ...
  <label for="delet" class="show_btn">Send Mail</label></h2>
 		...
 		<form action="register_other.php" method="post">
 			<h1><u>Register A New Civil Case Here</u></h1>
