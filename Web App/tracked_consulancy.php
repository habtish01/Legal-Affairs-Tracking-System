...
$sql="select *from consulancy where status=2";
$data=mysqli_query($conn,$sql);
if(isset($_POST['update'])){
	$id=$_POST['id'];
	$status=$_POST['status'];
	$sql="update consulancy set body='$status' where case_id='$id'";
		$run=mysqli_query($conn,$sql);

		if($run){
			echo '<script type="text/javascript">';
    	   	  	   echo 'alert("You Successfully Updated");';
    	   	  	   echo 'window.location.href="tracked_consulancy.php"';
    	   	  	   echo '</script>';
	...
	$size=$_FILES['file']['size'];
		...
		if(move_uploaded_file($file, $destination)){
			$sq="insert into files(file_name,size,download,status) values('$filename.','$size',0,9)";
...
if (isset($_POST['finish_track'])) {
	$id=$_POST['id'];
	$sql="update  consulancy set status=3 where case_id='$id'";
	$run=mysqli_query($conn,$sql);
	if($run){
		echo '<script type="text/javascript">';
...
<head>
	<meta charset="utf-8">
	<title>This Is Tracked Consulancy Page</title>
...
<div class="container">
    ...
    <label for="delet" class="close_btn" title="close">&times;</label>
     <h1>Delete Case</h1>
    <form class="delete" action="tracked_consulancy.php" method="post">
  ...
  <div class="container3">
    ...
    <label for="upload" class="close_btn" title="close">&times;</label>
     <h1>Upload File</h1>
    	<form class="delete" action="tracked_consulancy.php" method="post" enctype="multipart/form-data">
    ...
    <div class="content">
		...
		if($data->num_rows >0){
			?>
			 <h1><strong>Avialable Tracked Consultancy Cases</strong></h1>
			 <?php
...
<input type="checkbox" id="start">
  ...
  <label for="remined" class="show_btn">Set Reminder</label>
    <br>
    	<a href="consulancy_register.php">Edit</a>
                 ...
                 echo'<h1 class="no"><strong>No AvialableTracked Consulancy Cases</strong></h1>';
                  ?>
  ...
  <label for="upload" class="show_btn">Upload File</label>
    	...
    	<?php
    		...
    		$sqll="select *from files where status=9";
